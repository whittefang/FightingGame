...
public class CharacterAttackScriptGrap : MonoBehaviour {
	bool airmove;
	...
	public float ReflectStartup;
	public float ReflectRecovery;
	...
	void Start () {
		...
		CC.SetAttackFunctions (Reflect, NormalMove, Uppercut, Throw, Parry, Sweep, Super ,cancelStartup);
		facing = GameObject.Find ("MatchHandler").GetComponent<FacingScript> ();
	...
	public void Reflect(){
		if (CSS.GetState() == 1) {
			CSS.SetState(4, ReflectRecovery);
			Invoke("UseReflect", ReflectStartup);
		}
	}
	void UseReflect(){
		ReflectObj.transform.localPosition = new Vector3 (3f, 0, 0);
		ReflectObj.transform.rotation = Quaternion.Euler(0f, 0f, 0f);
		ReflectObj.gameObject.SetActive (true);
	...
	void UseUppercut(){
		...
		if (facing.getDirection (Player1)) {
			RB.AddForce(new Vector2(600, 1000));
		}else {
			RB.AddForce(new Vector2(-600, 1000));
		}
